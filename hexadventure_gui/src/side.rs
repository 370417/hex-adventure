...
use ggez::graphics::{Color, Drawable, DrawParam, Point2, Mesh, DrawMode};
...
impl Sidebar {
    ...
    pub fn draw(&self, ctx: &mut Context, dest: Point2) -> GameResult<()> {
        ...
        let background = [Point2::new(0.0, 0.0), Point2::new(width, 0.0),
            Point2::new(width, height), Point2::new(0.0, height)];
        let background = Mesh::new_polygon(ctx, DrawMode::Fill, &background)?;
        background.draw_ex(ctx, DrawParam {
            dest,
